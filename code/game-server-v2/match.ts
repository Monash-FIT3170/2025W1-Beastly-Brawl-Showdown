...
import {
  COMMON_MOVE_NAMES,
  COMMON_MOVE_POOL,
} from "../simulator/data/common/common_move_pool";
...
export class Match {
  ...
  createBattle(): void {
    ...
    const p1Template =
      COMMON_MONSTER_POOL.monsters[
        this.player1
          .selectedMonsterTemplateName as keyof typeof COMMON_MONSTER_POOL.monsters
      ];
    ...
    if (!p1Template || !p2Template) {
      throw new Error(
        `Match ${this.matchID}: Could not find template for one or both players.`
      );
    }
  ...
  submitMove(
    ...
    if (this.matchType === MatchType.BYE || !this.battle) {
      throw new Error(
        `Match ${this.matchID} has no battle to submit moves to.`
      );
    }

    console.log(
      `[MATCH DEBUG] submitMove called for ${player.displayName} with moveId ${moveId}, targetMethod ${targetMethod}, targetSide ${targetSide}`
    );
    // Store move
    ...
    const chooseMoveNotice = noticeMap.get("chooseMove") as
      | ChooseMove
      | undefined;

    if (!chooseMoveNotice) {
      throw new Error(
        `Match ${this.matchID}: Player ${player.displayName} has no chooseMove notice.`
      );
    }
    ...
    console.log(
      `[MATCH DEBUG] Submitting move ${moveId} (${targetMethod}) for ${player.displayName}`
    );
  ...
  submitRoll(player: Player): void {
    if (this.matchType === MatchType.BYE || !this.battle) {
      throw new Error(
        `Match ${this.matchID} has no battle to submit rolls to.`
      );
    }
    ...
    if (!rollNotice) {
      throw new Error(
        `Match ${this.matchID}: Player ${player.displayName} has no roll notice.`
      );
    }
    rollNotice.callback();
  }
  ...
  submitReroll(player: Player, option: boolean): void {
    if (this.matchType === MatchType.BYE || !this.battle) {
      throw new Error(
        `Match ${this.matchID} has no battle to submit rerolls to.`
      );
    }

    const sideIndex = this.getSideForPlayer(player);
    const noticeMap = this.battle!.noticeBoard.noticeMaps[sideIndex];
    const rerollNotice = noticeMap.get("rerollOption") as
      | RerollOption
      | undefined;

    if (!rerollNotice) {
      throw new Error(
        `Match ${this.matchID}: Player ${player.displayName} has no reroll notice.`
      );
    }
  ...
  getMonsterAbility(player: Player): COMMON_MOVE_NAMES | null {
    ...
    const template =
      this.battle.monsterPool.monsters[
        monster.baseID as keyof typeof this.battle.monsterPool.monsters
      ];
