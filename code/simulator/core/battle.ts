...
import { AbilityChargeComponent } from "./monster/component/core_components";
    ...
    this.sides = options.playerOptionSet.map((playerOptions, idx) => {
      if (!this.monsterPool.monsters[playerOptions.monsterId]) {
        throw new RangeError(
          `Key out of range: [key=${playerOptions.monsterId}] does not exist in the monster pool ${this.monsterPool.name}`
        );
      }
    ...
    while (this.sides.every((side) => side.monster.health > 0)) {
      ...
      await new Promise<void>((resolve) => {
        this.sides.forEach((side) => {
          ...
          if (!base) {
            throw new RangeError(
              `Key out of range: [key=${side.monster.baseID}] does not exist in the monster pool ${this.monsterPool.name}`
            );
          }
          ...
          const abilityChargeComponent: AbilityChargeComponent | null =
            getComponent(side.monster, "abilityCharges");
          if (
            base.abilityActionId &&
            abilityChargeComponent !== null &&
            abilityChargeComponent.charges > 0
          ) {
      ...
      const moveRequestQueue: MoveRequest[] = allMovesUnsorted.sort((a, b) => {
        ...
        return (
          getStat(
            "speed",
            this.sides[b.source].monster,
            this.monsterPool.monsters[this.sides[b.source].monster.baseID]!
          ) -
          getStat(
            "speed",
            this.sides[a.source].monster,
            this.monsterPool.monsters[this.sides[a.source].monster.baseID]!
          )
        );
      });
