...
describe("Battle.Move.perform.defend", () => {
  test("defend consumes a charge and grants temporary armour", async () => {
    ...
    try {
      ...
      const startingCharges = defender.monster.attackCharges;
      defender.monster.attackCharges = Math.max(0, startingCharges - 1);
      ...
      expect(defender.monster.attackCharges).toBe(startingCharges);
